...
const Projects = () => {
    ...
    useEffect(() => {
        axios.get((process.env.REACT_APP_API_URL ?? `https://cpm.tin12q.org/`) + `api/projects?page=${page}`, {
    ...
    useEffect(() => {
        axios.get((process.env.REACT_APP_API_URL ?? `https://cpm.tin12q.org/`) + `api/projects/search?search=${search}`, {
            headers: { Authorization: `Bearer ${cookies.token}` }
        })
            .then(res => {
                setProjects(res.data);
