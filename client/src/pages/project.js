...
function Project() {
    ...
    useEffect(() => {
        ...
        axios.get((process.env.REACT_APP_API_URL ?? `https://cpm.tin12q.org/`) + `api/projects/${id}`, { headers: { Authorization: `Bearer ${cookies.token}` } })
            .then(response => {
                setProjectData(response.data);

            })
            .catch(error => console.error(error));
        axios.get((process.env.REACT_APP_API_URL ?? `https://cpm.tin12q.org/`) + `api/tasks/project/${id}`, { headers: { Authorization: `Bearer ${cookies.token}` } })
            .then(res => {
