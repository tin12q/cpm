	...
	useEffect(() => {
		...
		axios
			...
			.then((res) => {
				...
				setDifficulty(res.data.difficulty || 2);
				setPriority(res.data.priority || 3);
				setCanParallelize(res.data.can_parallelize !== false);
			})
			.catch((err) => {
				alert(err);
			});
			...
			<form onSubmit={handleSubmit}>
				...
				<div className="mb-5">
					<Select
						...
						onChange={(val) => setDifficulty(parseInt(val))}
						size="lg"
					>
				...
				<div className="mb-5">
					<Select
						...
						onChange={(val) => setPriority(parseInt(val))}
						size="lg"
					>
				...
				<div className="mb-5 flex items-center gap-3">
					<label className="text-sm font-medium">Can Parallelize:</label>
					<input
						type="checkbox"
						checked={canParallelize}
						onChange={(e) => setCanParallelize(e.target.checked)}
						className="w-5 h-5"
					/>
				</div>
