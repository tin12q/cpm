...
export default function TaskComp(props) {
	...
	useEffect(() => {
		axios
			.get("http://localhost:1337/api/users/getAll", {
		...
		axios
			.get(
				(process.env.REACT_APP_API_URL ?? `http://localhost:1337/`) +
					`api/tasks/project/${idt}?page=${page}`,
				{ headers: { Authorization: `Bearer ${cookies.token}` } }
			)
			.then((res) => {
				setTasks(res.data);
	...
	useEffect(() => {
		axios
			.get(
				(process.env.REACT_APP_API_URL ?? `http://localhost:1337/`) +
					`api/tasks/name/?name=${search}&project=${idt}`,
				{ headers: { Authorization: `Bearer ${cookies.token}` } }
			)
			.then((res) => {
				setTasks(res.data);
